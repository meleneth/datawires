  ...
  class BodyValidator
    ...
    def validate_entries(key, kinds)
      ...
      entries.each_with_index do |entry, index|
        ...
        validate_relationship_collection(entry, index) if key == "sections" &&
          %w[membership_collection role_assignment_collection].include?(entry["kind"])
    ...
    def validate_relationship_collection(entry, index)
      prefix = "sections[#{index}].config"
      config = entry["config"]
      unless config.is_a?(Hash)
        errors << "#{prefix} must be an object"
